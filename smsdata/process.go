...
func BuildSortedSMS(in []SMSData) [][]SMSData {
	// 1) нормализуем страны (делаем копию входного среза)
	mapped := make([]SMSData, len(in))
	copy(mapped, in)
	for i := range mapped {
	...
	byProvider := make([]SMSData, len(mapped))
	copy(byProvider, mapped)

	slices.SortStableFunc(byProvider, func(a, b SMSData) int {
		return strings.Compare(a.Provider, b.Provider) //не учитывал strings.ToLower, может и стоит
	})

	// 3) сортировка по стране (A→Z)
	byCountry := make([]SMSData, len(mapped))
	copy(byCountry, mapped)

	slices.SortStableFunc(byCountry, func(a, b SMSData) int {
		return strings.Compare(a.Country, b.Country)
	})

	// 4) объединяем
	return [][]SMSData{byProvider, byCountry}
}
